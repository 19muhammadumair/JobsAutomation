...
export default function WhatsAppPanel() {
  ...
  useEffect(() => {
    fetchWhatsAppStatus()
      .then(setStatus)
      .catch(() => setStatus({ connected: false, chat_id: "" }));
  }, []);
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
      ...
      <div className="flex items-center gap-2 mb-3">
        <span
          className={`w-2 h-2 rounded-full ${status?.connected ? "bg-green-500" : "bg-red-400"}`}
        />
      ...
      {status?.chat_id && (
        <p className="text-xs text-gray-400 mb-3 truncate" title={status.chat_id}>
      ...
      {groups.length === 0 ? (
        ...
        <div className="space-y-2">
          <p className="text-xs font-medium text-gray-600">Select a group:</p>
